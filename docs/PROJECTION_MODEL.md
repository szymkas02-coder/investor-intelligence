...
## Current Model — Horizon-Dependent Weights

The projection model in `backend/routers/decision.py` (`get_projection()`) uses **continuously varying weights** based on the forecast horizon T (years). This replaced the original fixed-weight ensemble below.

```python
w_momentum(T)  = max(0, 1 - T/3) * 0.40        # dominates 1–3Y
w_cape(T)      = 0.55 * exp(-0.5 * ((T-5)/3)^2) # peaks at 5Y, Gaussian shape
w_base_rate(T) = 1 - w_momentum(T) - w_cape(T)  # dominates 10Y+
```

**Signals:**
- **Momentum (1–3Y):** ACWI 63d return, dampened when CAPE > 20 (high-valuation filter). Reference: Jegadeesh-Titman (1993).
- **CAPE (3–7Y):** Shiller E/P minus risk-free rate formula (60%) blended with Asness decile table (40%). Reference: Campbell-Shiller (1988, 1998).
- **Base rate (7–10Y+):** DMS 2025 historical equity premium, ~5% real. Reference: Dimson-Marsh-Staunton (2025).

**Why horizon-dependent:** Momentum is mean-reverting at 3+ years. CAPE is informative at 5–10 year horizons. Long-run base rate dominates at 10Y+. Fixed weights are a misspecification — they apply the wrong signal at each horizon.

---

## Original Fixed-Weight Design (historical)

> The section below documents the original ensemble design rationale. The weight values (30/50/20) are no longer used in production — they have been replaced by the horizon-dependent formula above.
